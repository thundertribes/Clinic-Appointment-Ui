...
interface DepartmentDischargeModalProps {
  isOpen: boolean
  onClose: () => void
  roomId: string
  roomNumber: string
...
export default function DepartmentDischargeModal({
  isOpen,
  onClose,
  roomId,
  roomNumber,
  ...
  const [requestCleaning, setRequestCleaning] = useState(true)
  ...
  const handleSubmit = () => {
    ...
    setTimeout(() => {
      ...
      window.location.href = "/rooms/departments"
    }, 1500)
  }
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-[500px]">
        <DialogHeader>
          ...
          <DialogDescription>
            Complete the discharge process for patient {patientName} from room {roomNumber} in {department} department.
          </DialogDescription>
        </DialogHeader>
        <div className="grid gap-4 py-4">
          ...
          <div className="flex items-center space-x-2">
            <Checkbox
              id="request-cleaning"
              checked={requestCleaning}
              onCheckedChange={(checked) => setRequestCleaning(checked as boolean)}
            />
            <Label htmlFor="request-cleaning" className="text-sm font-normal">
              Request room cleaning after discharge
            </Label>
          </div>
        </div>
        <DialogFooter>
