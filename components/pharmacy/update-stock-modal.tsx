    ...
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="sm:max-w-[500px]">
        ...
        <div className="grid gap-4 py-4">
          <RadioGroup value={operation} onValueChange={(value) => setOperation(value as "add" | "subtract")} className="grid grid-cols-2 gap-4">
            <div>
          ...
          <div className="grid gap-2">
            ...
            <Input id="quantity" type="number" min="1" value={quantity || ""} onChange={(e) => setQuantity(Number.parseInt(e.target.value) || 0)} />
          </div>
          ...
          <div className="grid gap-2">
            ...
            <Textarea id="notes" placeholder="Add any additional information" value={notes} onChange={(e) => setNotes(e.target.value)} />
          </div>
        </div>
        ...
        <DialogFooter>
          ...
          <Button onClick={handleSubmit} disabled={quantity <= 0 || isSubmitting || (operation === "add" && !batchNumber) || (operation === "add" && !expiryDate)}>
